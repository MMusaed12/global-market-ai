...
from common import (CFG, TWELVE_KEY, db, quote, pct, gdelt_news, td,
                    yahoo_quote, yahoo_history, analyze, refresh_seconds)
...
def saudi_history(symbol):
    ...
    df = pd.DataFrame({
        ...
        'open': q.get('open') or [None] * len(ts), 'high': q.get('high') or [None] * len(ts), 'low': q.get('low') or [None] * len(ts), 'close': q.get('close') or [None] * len(ts)
    })
    for c in ['open','high','low','close']:
        df[c] = pd.to_numeric(df[c], errors='coerce')
...
with st.sidebar:
    st.header('التحكم')
    refresh = st.select_slider('تحديث الواجهة', [300, 600, 900, 1800], value=refresh_seconds(CFG.get('refresh_seconds')), format_func=lambda x: f'{x//60} دقائق')
...
def render_dashboard():
    ...
    for item in CFG['watchlist']:
        if not TWELVE_KEY or item['symbol'] not in chosen_global:
            continue
        try:
    ...
    try:
        if market_type == 'عالمي':
            options = chosen_global if TWELVE_KEY else []
            if not options:
                st.info('اختر أصلًا متاحًا من القائمة الجانبية لعرض الرسم.')
                return
        ...
        if h.empty:
            st.info('لا تتوفر بيانات صالحة للرسم حاليًا.')
        else:
